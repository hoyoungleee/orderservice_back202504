...
public class SecurityConfig {
    ...
    //시큐리티 기본설정( 권한처리, 초기 로그인 화면 없애기 등등..)
    @Bean // 이 메서드가 리턴하는 시큐리티 설정을 빈으로 등록
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        // 나머지 요청들은 권한 검사가 필요하다고 설정했음
        // 권한 검사가 필요한 요청들을 어떤 필터로 검사할지를 추가해주면 됩니다.

        // 스프링 시큐리티가 기본으로 세팅하는 여러가지 필터
        // 내가 만든 커스텀 필터가 그 필터를 대체랗것이라 기존 필터 앞에 배치
        http.addFilterBefore(jwtAuthFilter, UsernamePasswordAuthenticationFilter.class);

        //인증과정에서 예외가 발생할 경우 그 예외를 핸들링할 객체를 등록
